-- INSERT INTO contact (contact_id, name, first_name, last_name, gender_id, dob, email, phone, address, photo_path, created_date, created_by)
-- VALUES
-- (1, 'John Doe', 'John', 'Doe', 1, '1990-01-01', 'john@example.com', '123-456-7890', '123 Main St', 'path/to/photo.jpg', '2024-04-16 12:00:00', 'Admin'),
-- (2, 'Jane Smith', 'Jane', 'Smith', 2, '1992-05-15', 'jane@example.com', '987-654-3210', '456 Oak St', 'path/to/photo2.jpg', '2024-04-16 12:30:00', 'Admin');

INSERT INTO account (name, status,balance)
VALUES
('John','Active',20000),
('Jane','Active',200000),
('Jasie','Active',30000);

-- INSERT INTO deposit_transaction (amount, status, account_id)
-- VALUES
-- ('John','Active',20000),
-- ('Jane','Active',200000),
-- ('Jasie','Active',30000);


